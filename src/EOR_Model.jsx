...
import { BarChart, Bar, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceLine, AreaChart, Area, PieChart, Pie, Cell } from 'recharts';
...
const EORModelApp = () => {
  ...
  // Default: 0.35 t/bbl (mature, ~6.7 MCF/bbl net) — mid-range for established Permian floods
  const [co2OilRatio, setCo2OilRatio] = useState(2.0); // tonnes CO₂ per barrel at steady-state (field data median ~5.4, weighted avg ~2)
  ...
  const patternDerived = useMemo(() => {
    ...
    const effectiveSpacing = Math.max(1, wellSpacing * patConfig.acresPerPattern); // Prevent div by 0
    
    // Max patterns field can support (minimum 1)
    const maxPatterns = Math.max(1, Math.floor((fieldSize * fieldShapeFactor) / effectiveSpacing));
    
    // Injectors per pattern from configuration
    const injPerPattern = patConfig.inj;
    const prodPerPatternConfig = patConfig.prod;
    
    // For single-well (H&P), same well is both injector and producer
    const isSingleWell = patternType === 'single-well';
    
    // 5-spot producer sharing: producers on corners are shared between adjacent patterns
    // For n patterns arranged in a grid:
    // - 1 pattern: 4 producers (no sharing)
    // - 2 patterns: 6 producers (share 2)
    // - 3 patterns: 8 producers (share 4)
    // - 4 patterns (2x2): 9 producers (share 7)
    // Formula: for a roughly square arrangement, producers ≈ (sqrt(n)+1)² for perfect squares
    // Simplified: effectiveProducers = 4 + (n-1) * 2 for linear, or use grid formula
    const getProducersForPatterns = (n) => {
      if (n <= 0) return 0;
      if (n === 1) return 4;
      // Approximate as grid: rows × cols arrangement
      const cols = Math.ceil(Math.sqrt(n));
      const rows = Math.ceil(n / cols);
      // Producers at grid intersections = (rows+1) × (cols+1), but only count for actual patterns
      return (rows + 1) * (cols + 1);
    };
    
    const effectiveProdPerPattern = isSingleWell 
      ? 0 // H&P: no separate producers
      : patternType.includes('inv') 
        ? prodPerPatternConfig  // Inverted patterns: producer in center, not shared
        : maxPatterns > 0 ? getProducersForPatterns(maxPatterns) / maxPatterns : 4;
    
    // Injectors needed to use full facility capacity
    const injectorsForFacility = Math.ceil(facilitySize / injRatePerWell);
    
    // Actual max injectors (limited by field)
    ...
    // Pattern lifecycle duration (fill + ramp + peak + decline to abandonment)
    const safeDeclineRate = Math.max(0.01, Math.min(99.99, patternDeclineRate)); // Prevent div by zero
    const declineYearsToAbandonment = patternAbandonmentPct < 100 && patternAbandonmentPct > 0
      ...
      : 20; // cap at 20 years if abandonment = 100% or 0%
    const patternLifeYears = patternFillYears + patternRampYears + patternPeakYears + Math.abs(declineYearsToAbandonment);
    
    // For existing flood: how many patterns already active
    const existingPatterns = Math.round(maxPatterns * (pctDeveloped / 100));
    
    // Patterns that can be developed simultaneously (limited by facility capacity)
    // Larger facility = can run more patterns = faster development
    const injectorsAtCapacity = Math.floor(facilitySize / injRatePerWell);
    const patternsAtCapacity = Math.max(1, Math.floor(injectorsAtCapacity / injPerPattern));
    
    // For WAG: staggered operation allows more patterns since they take turns with CO2
    // Number of stagger groups = 1 + wagRatio (e.g., 1:1 = 2 groups, 2:1 = 3 groups)
    // Each group gets CO2 in sequence, so facility can support more total patterns
    const wagStaggerMultiplier = eorMethod === 'wag' ? (1 + wagRatio) : 1;
    
    // For H&P: wells only inject during huff phase (~10% of cycle time)
    // But we don't want to start too many wells at once - cap at 3x multiplier
    // This represents practical limits on operations, not just CO2 supply
    ...
    // Starting patterns for new flood = what facility can support (with staggering for WAG/H&P)
    // Can't start more than field has or facility can supply
    const startingPatternsNew = Math.min(effectivePatternsAtCapacity, maxPatterns);
    
    // Development rate per phase
    const patternsPerPhase = Math.max(1, Math.min(effectivePatternsAtCapacity, maxPatterns)); 
    
    // Calculate project life based on scenario and field size
    let estProjectLife;
    const remainingPatterns = maxPatterns - existingPatterns;
    
    // Development time scales with number of patterns to develop and facility capacity
    // Larger facility (more patternsPerPhase) = faster development
    // More patterns (larger field) = longer development
    // Typical development: 6-12 months between adding new patterns
    const yearsPerPhase = 0.75; // 9 months between development phases (was 1.5)
    
    if (scenarioType === 'Existing') {
      // EXISTING FLOOD: 
      // - Existing patterns are already in various stages of their lifecycle
      // - We only need time for: remaining development + life of last pattern started
      // - If fully developed (100%), project life = remaining life of youngest pattern
      
      if (remainingPatterns <= 0) {
        // Fully developed - just run out existing patterns
        // Youngest pattern is at avgPatternAge, so remaining life is patternLifeYears - avgPatternAge
        const remainingPatternLife = Math.max(
          patternLifeYears - avgPatternAge,
          Math.abs(declineYearsToAbandonment) + 2 // At least decline + 2 years
        );
        estProjectLife = Math.ceil(remainingPatternLife);
      } else {
        // Partially developed - need to develop remaining patterns
        const phasesToComplete = Math.ceil(remainingPatterns / patternsPerPhase);
        const developmentTime = phasesToComplete * yearsPerPhase;
        // Project ends when last-started pattern completes its lifecycle
        estProjectLife = Math.ceil(developmentTime + patternLifeYears);
      }
    } else {
      // NEW FLOOD:
      // - Start with startingPatternsNew patterns
      // - Develop remaining patterns over time based on facility capacity
      // - Project life = full development time + pattern lifecycle
    ...
    // Minimum project life is pattern life (even small fields need full pattern lifecycle)
    ...
    // Is current field size adequate?
    const fieldSizeAdequate = fieldSize >= minOptimalFieldSize * 0.9;
    ...
    return {
      ...
      declineYearsToAbandonment: Math.round(Math.abs(declineYearsToAbandonment) * 10) / 10,
      existingPatterns,
      remainingPatterns,
      startingPatternsNew,
      patternsPerPhase,
      estProjectLife, // no cap - scales with field size and facility capacity
      // Optimal field size outputs
      patternsNeededForFacility,
      optimalPatternCount,
      minOptimalFieldSize,
      fieldSizeAdequate,
      isSingleWell, // H&P mode
    };
  }, [fieldSize, fieldShapeFactor, wellSpacing, facilitySize, injRatePerWell, 
      patternFillYears, patternRampYears, patternPeakYears, patternDeclineRate, 
      patternAbandonmentPct, pctDeveloped, patternType, avgPatternAge, scenarioType]);
  ...
  const resetToDefaults = () => {
    ...
    setEorMethod('continuous'); setWagRatio(1); setWagCycleMonths(3);
    setHpInjectionDays(30); setHpSoakDays(15); setHpProductionDays(60); setHpCycles(3);
    ...
    setNewWellsPct(0); // Default to all retrofits
    setRetrofitCostPerWell(0); setRetrofitWellCount(0);
    setCo2Price(-30); setRecycledCo2Cost(12.35);
    setProdOpex(10); setOilTransport(5); setSeveranceTax(8);
    setCreditYears(12); setCreditTotal(85); setCreditEnabled(false);
    setCreditInflation(false); setCreditInflationRate(2.5);
    setRecycleMode('fixed'); setFixedRecycleRate(60);
    setRampMode('auto');
    ...
    setFullValueChain(false); setAutoCalcCapex(true); setPipelineDistance(50);
    setCo2SourceType('ngp'); setCo2Concentration(96); setPipelineCostPerInchMile(85000);
    setCaptureCapex(0); setRecycleCapex(0); setStorageCapex(0); setTransportCapex(0);
  ...
  const calculatePatternMetrics = useCallback(() => {
    const pd = patternDerived;
    ...
    const netOilPrice = wtiPrice - oilDiff;
    const sevTaxRate = severanceTax / 100;
    // Use effective discount rate (either hurdle rate or WACC)
    const dr = computedFinancing.effectiveDiscountRate;
    
    // Pattern state: array of { startMonth, active }
    const patterns = [];
    // No arbitrary limit - simulation runs until all patterns are developed AND abandoned
    // Safety cap at 1000 years to prevent infinite loops
    const maxSimulationMonths = 1000 * 12;
    let lastPatternStartMonth = 0;
    
    // Initialize patterns for existing flood
    if (scenarioType === 'Existing') {
      for (let i = 0; i < pd.existingPatterns; i++) {
        // Spread existing patterns from (0.5 × avgPatternAge) to (1.5 × avgPatternAge)
        // This ensures all patterns are mature (past fill/ramp) while centering on the average
        const minAge = avgPatternAge * 0.5;
        const maxAge = avgPatternAge * 1.5;
        const ageRange = pd.existingPatterns > 1 ? (i / (pd.existingPatterns - 1)) : 0.5;
        const patternAge = minAge + ageRange * (maxAge - minAge);
        const startMonth = -Math.round(patternAge * 12); // negative = started in past
        patterns.push({ startMonth, active: true });
      }
    } else {
      // New flood: start with minimum patterns
      for (let i = 0; i < pd.startingPatternsNew; i++) {
    ...
    const getPatternRecycleFraction = (ageMonths) => {
      ...
      if (recycleMode === 'fixed') return fixedRecycleRate / 100;
    ...
    const getPatternProduction = (ageMonths, currentMonth) => {
      ...
      const peakProdPerMonth = baseInjPerMonth / co2OilRatio;
      ...
      if (eorMethod === 'huffpuff') {
        ...
        if (cycleNum >= totalCycles) {
          ...
          const co2Produced = blowdownProd * co2OilRatio * 0.4;
          return { 
        ...
        // WAG improves sweep efficiency - literature shows 5-25% incremental recovery
        // Model this as LOWER CO2/oil ratio (more oil per tonne CO2)
        const wagEfficiencyBoost = 1.15; // 15% better sweep = 15% more oil per CO2
        const wagCO2OilRatio = co2OilRatio / wagEfficiencyBoost; // Lower ratio = better efficiency
        
        // Peak production recalculated with WAG-adjusted ratio
        const wagPeakProdPerMonth = baseInjPerMonth / wagCO2OilRatio;
        ...
        if (inBlowdown) {
          ...
          // CO2 and water still come out during blowdown
          const co2Produced = baseProd * wagCO2OilRatio * getPatternRecycleFraction(ageMonths);
          const waterProduced = baseProd * 0.6; // Higher water cut in WAG blowdown
          return {
            oil: baseProd,
            co2Inj: 0,
            waterInj: 0,
        ...
        // WAG operations - use CYCLE-AVERAGED values for smoother financial modeling
        // In reality, facilities use buffer storage to smooth out cyclic operations
        // Instantaneous values would show high/zero alternation, but net effect is averaged
        
        // Cycle-averaged injection fractions
        ...
        // Production is relatively stable (slight dip during water phase averaged out)
        const avgPhaseMod = co2FractionOfCycle * 1.0 + waterFractionOfCycle * 0.85;
        
        // Cycle-averaged injection rates
        const avgCo2Inj = baseInjPerMonth * co2FractionOfCycle;
        const avgWaterInj = baseInjPerMonth * wagRatio * waterFractionOfCycle;
        
        // CO2 produced comes back with oil production continuously (use WAG-adjusted ratio)
        const co2Produced = baseProd * avgPhaseMod * wagCO2OilRatio * getPatternRecycleFraction(ageMonths);
        ...
        const waterProduced = (baseProd * avgPhaseMod * 0.5) + (avgWaterInj * waterBreakthroughFrac * 0.5);
        
        // Determine current phase for display purposes only
        const isWaterPhase = monthInCycle >= co2PhaseEnd;
        
        return {
          oil: baseProd * avgPhaseMod,
          co2Inj: avgCo2Inj,
          ...
          isInjecting: true,
          isProducing: true,
          phase: isWaterPhase ? 'water' : 'co2' // For display only
        };
      } else {
        ...
        if (inBlowdown) {
          ...
          const co2Produced = baseProd * co2OilRatio * getPatternRecycleFraction(ageMonths);
          return {
            oil: baseProd,
            co2Inj: 0,
            waterInj: 0,
    ...
    const facilityMtYr = facilitySize / 1e6;
    const facilityDailyInj = facilitySize / 365;
    const facilityMcfd = facilityDailyInj * 19;
    
    if (scenarioType === 'New') {
      // Recycle CAPEX (only if recycle plant doesn't exist)
      let patternRecycleCapex = 0;
      if (!recycleExists) {
        if (facilitySize < 579000) {
          patternRecycleCapex = (1200 * facilityMcfd / 1e6) * 1.5;
        } else {
          patternRecycleCapex = ((36000 + (((facilityMcfd / 1000) - 30) * 750)) / 1000) * 1.5;
        }
      }
      
      // Storage CAPEX - minimal, mainly MMV equipment
      const patternStorageCapex = 0.5; // $0.5MM
      
      ccusCapexVal = (patternRecycleCapex + patternStorageCapex) * 1e6;
    }
    // Existing flood: no recycle/storage CAPEX (sunk cost)
    
    // For Full Value Chain (both new and existing): add capture and transport CAPEX
    if (fullValueChain) {
      // Capture CAPEX: scale based on facility size using same formula as computedCapex
      const source = CO2_SOURCES[co2SourceType] || CO2_SOURCES.ngp;
      const baseCapexPerTpa = co2SourceType === 'custom'
        ? capexFromConcentration(co2Concentration)
        : source.capexPerTpa;
      ...
      const patternCaptureCapex = (scaledCapexPerTpa * facilitySize) / 1e6; // $MM
      
      // Transport CAPEX: pipeline sized for facility throughput
      const pipeDiameterIn = Math.max(4, Math.min(36, 5.35 * Math.pow(Math.max(facilityMtYr, 0.01), 0.45)));
      const nominalDiameter = Math.ceil(pipeDiameterIn / 2) * 2;
      const dist = Math.max(pipelineDistance, 1);
      const patternTransportCapex = (nominalDiameter * dist * pipelineCostPerInchMile * 1.15) / 1e6; // $MM
      
      ccusCapexVal += (patternCaptureCapex + patternTransportCapex) * 1e6;
    }
    ...
    if (scenarioType === 'New') {
      ...
      const initialPatterns = pd.startingPatternsNew;
    ...
    for (let m = 1; m <= maxSimulationMonths; m++) {
      ...
      // Calculate total field injection and production this month
      let fieldInjection = 0;
      let fieldProduction = 0;
      let activePatternCount = 0;
      let fieldRecycledCO2 = 0;
      ...
      for (let i = 0; i < patterns.length; i++) {
        ...
        // Track CO2 produced (comes back with oil) - used for H&P and blowdown phases
        if (patResult.co2Produced > 0) {
          fieldRecycledCO2 += patResult.co2Produced;
        }
        ...
        if (patResult.isInjecting) {
          ...
          if (patResult.co2Inj > 0) {
            ...
            fieldRecycledCO2 += patRecycled; // This is recycle from continuous injection
      ...
      const facilityFreshCap = facilitySize / 12; // Monthly fresh CO2 from facility
      ...
      // Add pattern if spare capacity exceeds threshold AND field has room
      const canAddPattern = availableFreshCO2Annual >= activationThreshold && patterns.length < pd.maxPatterns;
      ...
      if (allPatternsDeveloped && allPatternsAbandoned) {
        actualProjectMonths = m;
        break;
      }
      
      // Safety exit: if no patterns active and none can be added, we're done
      if (activePatternCount === 0 && !canAddPattern && m > 24) {
        actualProjectMonths = m;
        break;
      }
      ...
      if (co2Price > 0) {
        co2ReceiptRevenue = netNewCO2 * co2Price;
        totalCO2ReceiptRevenue += co2ReceiptRevenue;
      } else {
        co2PurchaseCost = netNewCO2 * Math.abs(co2Price);
        totalCO2PurchaseCost += co2PurchaseCost;
      }
      
      // OPEX - based on scaled production (what we actually produce)
      const oilOpexCost = scaledProduction * prodOpex;
      const oilTransCost = scaledProduction * oilTransport;
      const sevTaxCost = oilRev * sevTaxRate;
      const recycleOpexCost = actualRecycled * recycleOpex;
      const captureOpexCost = fullValueChain ? netNewCO2 * captureOpex : 0;
      const storageOpexCost = co2Stored * storageOpex;
      const transportOpexCost = fullValueChain ? netNewCO2 * transportOpex : 0;
      ...
      // Record monthly data (quarterly + first/last)
      if (m % 3 === 0 || m === 1 || m === actualProjectMonths) {
        // Calculate instantaneous CO2/Oil ratio (t/bbl) - capped for display
        const instantRatio = scaledProduction > 0 ? actualInjection / scaledProduction : co2OilRatio;
        const displayRatio = Math.min(instantRatio, co2OilRatio * 10); // cap at 10x base for display
        
        monthlyData.push({
          month: m,
          year,
          label: `Y${year}`,
          eorProd: scaledProduction,
          ...
          co2Injection: actualInjection,
          co2Fresh: freshCO2Supplied, // Fresh CO2 from facility
          co2Recycled: actualRecycled, // Recycled CO2
          waterInjection: fieldWaterInjection, // For WAG
          waterProduced: fieldWaterProduced, // For WAG
          recycleFraction: avgRecycleFrac * 100,
          activePatterns: patterns.filter(p => p.active).length,
          totalPatterns: patterns.length,
          cumulativeInjectors,
          cumulativeProducers,
          monthlyCapex: monthlyCapex / 1e6,
  ...
  }, [patternDerived, wtiPrice, oilDiff, severanceTax, discountRate, scenarioType,
      avgPatternAge, injRatePerWell, co2OilRatio, patternFillYears, patternRampYears,
      patternPeakYears, patternDeclineRate, patternAbandonmentPct, facilitySize,
  ...
  const oilPriceSensitivity = useMemo(() => {
    ...
    return prices.map(price => ({
      price,
      npv: calculateProjectMetrics({ wtiPrice: price }).projectNpv,
    }));
  ...
  const co2PriceSensitivity = useMemo(() => {
    ...
    return prices.map(price => ({
      price,
      npv: calculateProjectMetrics({ co2Price: price }).projectNpv,
  ...
  const capexBreakdown = useMemo(() => {
    ...
    if (captureCapex > 0) items.push({ name: 'Capture CAPEX', value: captureCapex });
    if (recycleCapex > 0) items.push({ name: 'Recycle CAPEX', value: recycleCapex });
    if (storageCapex > 0) items.push({ name: 'Storage CAPEX', value: storageCapex });
    if (transportCapex > 0) items.push({ name: 'Transport CAPEX', value: transportCapex });
    return items;
  }, [calcs, captureCapex, recycleCapex, storageCapex, transportCapex]);
    ...
    <div className="h-screen bg-gray-100 text-gray-800 font-sans text-sm flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-2 sm:p-3">
          ...
          <div className="grid gap-3" style={{gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))'}}>
              ...
              <>
                <div>
                    ...
                    <div className={`mt-1 pt-1 border-t ${patternDerived.fieldSizeAdequate ? 'border-gray-200' : 'border-amber-300'}`}>
                      ...
                      {!patternDerived.fieldSizeAdequate && (
                        <div className="text-xs text-amber-600 px-1 py-1 bg-amber-50 rounded mt-1" style={{fontSize:'10px'}}>
                          ⚠️ Field may be undersized for {(facilitySize/1000).toFixed(0)}k t/yr facility. Consider larger field or smaller facility.
                        </div>
                      )}
                      {patternDerived.fieldSizeAdequate && (
                  ...
                  <Section title={eorMethod === 'huffpuff' ? 'H&P Cycle Parameters' : 'Pattern Lifecycle'}>
                    ...
                    <div className="mt-1 pt-1 border-t border-gray-200">
                      <OutputRow label="Project Life (Sim)" value={calcs.actualProjectYears || '--'} unit="yrs" highlight
                        tooltip="Actual project duration from simulation - includes full field development and pattern lifecycles" />
                ...
                <div>
                  ...
                  <Section title="CO₂ / Oil Ratio">
                    <InputField label="Steady-State Ratio" value={co2OilRatio} onChange={setCo2OilRatio} unit="t/bbl" step={0.1}
                      tooltip="Tonnes CO₂ per barrel at peak production. Field data: median 5.4, weighted avg ~2 t/bbl." />
                ...
                <div>
                  ...
                  <Section title="Tax & Credits">
                    {/* State Selection */}
                    <div className="flex items-center justify-between py-1">
                      <label className="text-gray-700 text-xs font-medium">State</label>
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-200">
                      <div className="flex items-center justify-between py-1">
                        <label className="text-gray-700 text-xs font-medium">45Q CO₂ Credit</label>
                        <button onClick={() => setCreditEnabled(!creditEnabled)}
                          className={`w-8 h-4 rounded-full transition-colors relative ${creditEnabled ? 'bg-green-600' : 'bg-gray-300'}`}>
                          <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${creditEnabled ? 'translate-x-4' : 'translate-x-0.5'}`} />
                        </button>
                      </div>
                      {creditEnabled && (
                        <>
                          ...
                          <div className="flex items-center justify-between py-1">
                            <label className="text-gray-700 text-xs cursor-help border-b border-dotted border-gray-400"
                              title="Escalate 45Q credit value with inflation each year">Inflation Escalation</label>
                            <button onClick={() => setCreditInflation(!creditInflation)}
                              className={`w-8 h-4 rounded-full transition-colors relative ${creditInflation ? 'bg-green-600' : 'bg-gray-300'}`}>
                              <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${creditInflation ? 'translate-x-4' : 'translate-x-0.5'}`} />
                            </button>
                          </div>
                          {creditInflation && (
                    ...
                    {enableTaxCalc && (
                      <>
                        ...
                        <div className="mt-2 pt-2 border-t border-gray-200">
                          <div className="flex items-center justify-between py-1">
                            <label className="text-gray-700 text-xs font-medium cursor-help border-b border-dotted border-gray-400"
                              title="15% credit on EOR costs. Phased out when oil price > ~$59/bbl">§43 EOR Credit</label>
                            <button onClick={() => setSection43Enabled(!section43Enabled)}
                              className={`w-8 h-4 rounded-full transition-colors relative ${section43Enabled ? 'bg-green-600' : 'bg-gray-300'}`}>
                              <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${section43Enabled ? 'translate-x-4' : 'translate-x-0.5'}`} />
                            </button>
                          </div>
                        ...
                        <div className="mt-2 pt-2 border-t border-gray-200">
                          ...
                          <OutputRow label="State Income Tax" value={`${stateTaxRate.toFixed(1)}%`} />
            ...
            <div>
              ...
              <Section title="Tax & Credits">
                ...
                <div className="mt-2 pt-2 border-t border-gray-200">
                  <div className="flex items-center justify-between py-1">
                    <label className="text-gray-700 text-xs font-medium">45Q CO₂ Storage Credit</label>
                    <button onClick={() => setCreditEnabled(!creditEnabled)}
                      className={`w-8 h-4 rounded-full transition-colors relative ${creditEnabled ? 'bg-green-600' : 'bg-gray-300'}`}>
                      <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${creditEnabled ? 'translate-x-4' : 'translate-x-0.5'}`} />
                    </button>
                  ...
                  {creditEnabled && (
                    <>
                      ...
                      <div className="flex items-center justify-between py-1">
                        <label className="text-gray-700 text-xs cursor-help border-b border-dotted border-gray-400"
                          title="Escalate 45Q credit rate annually with inflation">Inflation Escalation</label>
                        <button onClick={() => setCreditInflation(!creditInflation)}
                          className={`w-8 h-4 rounded-full transition-colors relative ${creditInflation ? 'bg-green-600' : 'bg-gray-300'}`}>
                          <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${creditInflation ? 'translate-x-4' : 'translate-x-0.5'}`} />
                        </button>
                ...
                {enableTaxCalc && (
                  <>
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-200">
                      <div className="flex items-center justify-between py-1">
                        <label className="text-gray-700 text-xs font-medium cursor-help border-b border-dotted border-gray-400"
                          title="Section 43: 15% credit on qualified EOR costs. Phases out when oil price exceeds threshold.">§43 EOR Credit</label>
                        <button onClick={() => setSection43Enabled(!section43Enabled)}
                          className={`w-8 h-4 rounded-full transition-colors relative ${section43Enabled ? 'bg-green-600' : 'bg-gray-300'}`}>
                          <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${section43Enabled ? 'translate-x-4' : 'translate-x-0.5'}`} />
                        </button>
              ...
              <Section title="CO₂ / Oil Ratio" >
                <InputField label="Steady-State Ratio" value={co2OilRatio} onChange={setCo2OilRatio} unit="t/bbl" step={0.1}
                  tooltip="Tonnes of CO₂ injected per barrel of incremental oil at steady-state. Field data: median 5.4 t/bbl, P25-P75: 1.3-20.9 t/bbl, weighted avg ~2 t/bbl (higher-producing wells have better ratios). Literature optimized floods: 0.25-0.55 t/bbl." />
                <OutputRow label="Ratio (MCF/bbl)" value={`${(co2OilRatio * 19.25).toFixed(1)}`}
                ...
                {fullValueChain && (
                  <div className="flex items-center justify-between py-1">
                    ...
                    <button onClick={() => setAutoCalcCapex(!autoCalcCapex)}
                      className={`w-8 h-4 rounded-full transition-colors relative ${autoCalcCapex ? 'bg-green-600' : 'bg-gray-300'}`}>
                      <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${autoCalcCapex ? 'translate-x-4' : 'translate-x-0.5'}`} />
                    </button>
                  </div>
                )}
          ...
          <div className="grid gap-3" style={{gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))"}}>
            ...
            <div>
              <Section title="CAPEX Breakdown" >
                {calcs.prodWellCapexTotal > 0 && <OutputRow label="Production Wells" value={`$${calcs.prodWellCapexTotal.toFixed(1)}MM`} />}
                <OutputRow label="Injection Wells" value={`$${calcs.injWellCapex.toFixed(1)}MM`} />
                {calcs.wellRetrofitCapex > 0 && <OutputRow label="Well Retrofit" value={`$${calcs.wellRetrofitCapex.toFixed(1)}MM`}
                  tooltip="Converting existing wells for CO₂ service (tubing, wellhead, cement, packer)" />}
                ...
                <OutputRow label="CCUS Infrastructure" value={`$${calcs.ccusCapex.toFixed(1)}MM`} />
                <OutputRow label="Total CAPEX" value={`$${calcs.totalCapex.toFixed(1)}MM`} highlight />
              </Section>

              <Section title="OPEX Breakdown" >
                <OutputRow label="Oil Production" value={`$${calcs.oilOpex.toFixed(1)}MM`} />
                <OutputRow label="Oil Transport" value={`$${calcs.oilTransportCost.toFixed(1)}MM`} />
                <OutputRow label="Severance Tax" value={`$${calcs.severanceTaxCost.toFixed(1)}MM`} />
                {calcs.co2PurchaseCost > 0 && <OutputRow label="CO₂ Purchase" value={`$${calcs.co2PurchaseCost.toFixed(1)}MM`} />}
                <OutputRow label="CO₂ Recycle" value={`$${calcs.recycleOpexCost.toFixed(1)}MM`} />
                {calcs.captureOpexCost > 0 && <OutputRow label="CO₂ Capture" value={`$${calcs.captureOpexCost.toFixed(1)}MM`} />}
                <OutputRow label="CO₂ Storage" value={`$${calcs.storageOpexCost.toFixed(1)}MM`} />
                <OutputRow label="CO₂ Transport" value={`$${calcs.transportOpexCost.toFixed(1)}MM`} />
                <OutputRow label="Total OPEX" value={`$${calcs.totalOpex.toFixed(1)}MM`} highlight />
              </Section>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded p-3">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={calcs.monthlyData?.map(d => ({
                  ...
                  <Bar dataKey="co2ReceiptRevenue" name="CO₂ Receipt" stackId="pos" fill="#06b6d4" />
        ...
        {activeTab === 'sensitivity' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-6xl">
            <div className="bg-white border border-gray-200 rounded p-3">
              <h3 className="text-xs font-semibold text-gray-700 uppercase mb-2">NPV vs Oil Price</h3>
              <ResponsiveContainer width="100%" height={320}>
                <LineChart data={oilPriceSensitivity}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                  <XAxis dataKey="price" tick={{ fill: "#6b7280", fontSize: 10 }} label={{ value: 'WTI ($/bbl)', position: 'insideBottom', offset: -5, fill: "#6b7280", fontSize: 10 }} />
                  <YAxis tick={{ fill: "#6b7280", fontSize: 10 }} tickFormatter={v => `$${v.toFixed(0)}M`} />
                  <Tooltip contentStyle={{ backgroundColor: "#fff", border: "1px solid #e5e7eb", borderRadius: '6px', fontSize: '11px' }}
                    formatter={(v, name) => [`$${v.toFixed(1)}MM`, name]} />
                  <ReferenceLine y={0} stroke="#6b7280" strokeDasharray="3 3" />
                  <ReferenceLine x={wtiPrice} stroke="#15803d44" strokeDasharray="3 3" label={{ value: 'Current', fill: '#15803d', fontSize: 9 }} />
                  <Line type="monotone" dataKey="npv" name="EOR NPV" stroke="#15803d" strokeWidth={2} dot={{ r: 3, fill: '#15803d' }} />
                  <Legend wrapperStyle={{ fontSize: '10px' }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded p-3">
              <h3 className="text-xs font-semibold text-gray-700 uppercase mb-2">NPV vs CO₂ Price</h3>
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={co2PriceSensitivity}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                  <XAxis dataKey="price" tick={{ fill: "#6b7280", fontSize: 10 }} label={{ value: 'CO₂ Price ($/t)', position: 'insideBottom', offset: -5, fill: "#6b7280", fontSize: 10 }} />
                  <YAxis tick={{ fill: "#6b7280", fontSize: 10 }} tickFormatter={v => `$${v.toFixed(0)}M`} />
                  <Tooltip contentStyle={{ backgroundColor: "#fff", border: "1px solid #e5e7eb", borderRadius: '6px', fontSize: '11px' }}
                    formatter={(v) => [`$${v.toFixed(1)}MM`]} />
                  <ReferenceLine y={0} stroke="#6b7280" strokeDasharray="3 3" />
                  <Bar dataKey="npv" name="Project NPV" fill="#14b8a6">
                    {co2PriceSensitivity.map((entry, i) => (
                      <Cell key={i} fill={entry.npv >= 0 ? '#14b8a6' : '#ef4444'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="bg-white border border-gray-200 rounded p-3 lg:col-span-2">
              <h3 className="text-xs font-semibold text-gray-700 uppercase mb-2">NPV Sensitivity Table ($MM) — Oil Price vs CO₂ Price</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-xs">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="border border-gray-300 px-2 py-1 text-gray-400">WTI ↓ / CO₂ →</th>
                      {[-30, 0, 30, 60, 85].map(cp => (
                        <th key={cp} className="border border-gray-300 px-2 py-1 text-gray-400">{cp < 0 ? `Buy $${-cp}` : cp === 0 ? 'Free' : `+$${cp}`}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[40, 50, 60, 65, 70, 80, 90, 100].map(op => (
                      <tr key={op}>
                        <td className={`border border-gray-300 px-2 py-1 font-semibold ${op === wtiPrice ? 'bg-green-50 text-green-700' : 'text-gray-400'}`}>${op}</td>
                        {[-30, 0, 30, 60, 85].map(cp => {
                          const npv = calculateProjectMetrics({ wtiPrice: op, co2Price: cp }).projectNpv;
                          return (
                            <td key={cp} className={`border border-gray-300 px-2 py-1 text-right font-mono ${npv >= 0 ? 'text-green-600' : 'text-red-600'} ${op === wtiPrice && cp === co2Price ? 'bg-green-50' : ''}`}>
                              {npv.toFixed(0)}
                            </td>
                          );
                        })}
                  ...
                  <div className="mt-3">
                    ...
                    <div className="w-full overflow-auto">
                      {(() => {
                        ...
                        // Scale patterns to fit - allow up to 50 patterns to display
                        const maxPat = Math.min(patternDerived.maxPatterns || 0, 50);
                        const cols = Math.min(Math.ceil(Math.sqrt(maxPat * 1.2)), 10); // wider than tall
                        const rows = Math.ceil(maxPat / cols);
                        
                        // Calculate spacing based on number of patterns
                        // Smaller spacing for more patterns
                        const baseSpacing = patternType === 'single-well' ? 50 : 60;
                        const spacing = maxPat > 20 ? baseSpacing * 0.7 : (maxPat > 10 ? baseSpacing * 0.85 : baseSpacing);
                        
                        // Calculate SVG dimensions based on field
                        const fieldWidth = cols * spacing + 120;
                        const fieldHeight = rows * spacing + 100;
                        const svgWidth = Math.max(500, Math.min(800, fieldWidth + 100));
                        const svgHeight = Math.max(300, Math.min(600, fieldHeight + 80));
                        
                        const baseX = 90;
                        const baseY = 50;
                        
                        return (
                          <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full border border-gray-300 rounded bg-gradient-to-br from-amber-50 to-amber-100" style={{minWidth: '500px', maxHeight: '500px'}}>
                            {/* Field boundary */}
                            ...
                            <text x={baseX + cols * spacing / 2} y={baseY - spacing/2 - 18} 
                              textAnchor="middle" fontSize="10" fill="#92400e" fontWeight="bold">
                              {fieldSize} acres ({sqMiles.toFixed(2)} sq mi)
                            </text>
                            
                            {/* Main trunk pipeline */}
                            <line x1={15} y1={svgHeight/2} x2={baseX - spacing/2 - 10} y2={svgHeight/2} stroke="#1e40af" strokeWidth="6" />
                            <line x1={baseX - spacing/2 - 10} y1={baseY} x2={baseX - spacing/2 - 10} y2={baseY + rows * spacing} stroke="#1e40af" strokeWidth="4" />
                            <text x={8} y={svgHeight/2 - 8} fontSize="8" fill="#1e40af" fontWeight="bold">Trunk</text>
                            ...
                            {(() => {
                              ...
                              const elements = [];
                              const wellRadius = maxPat > 20 ? 5 : (maxPat > 10 ? 6 : 7);
                              const injRadius = wellRadius + 2;
                                ...
                                for (let idx = 0; idx < maxPat; idx++) {
                                  ...
                                  elements.push(
                                    <g key={`pattern-bg-${idx}`} opacity={state.isDeveloped ? 1 : 0.25}>
                                      ...
                                      {maxPat <= 25 && (
                                        <text x={cx} y={cy + spacing/2 + 8} textAnchor="middle" fontSize="6" 
                            ...
                            <g transform={`translate(${svgWidth - 75}, 15)`}>
                              <rect x={0} y={0} width={70} height={eorMethod === 'wag' ? 115 : 100} fill="white" fillOpacity="0.95" stroke="#e5e7eb" rx="4" />
                  ...
                  <div className="bg-white border border-gray-200 rounded p-3">
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <LineChart data={calcs.monthlyData || []}>
                        ...
                        <Line type="monotone" dataKey="co2Recycled" stroke="#f59e0b" strokeWidth={2} dot={false} name="CO₂ Produced (Recycled)" />
                        <Line type="monotone" dataKey="co2Stored" stroke="#16a34a" strokeWidth={2} dot={false} name="CO₂ Stored" />
                        {calcs.monthlyData?.length > 0 && (
